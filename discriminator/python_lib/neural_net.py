...
class NeuralNet(object):
    ...
    def linear(self, x, shape, name=None, l2_decay=None, batch_norm=False,
        ...
        with tf.variable_scope(name, reuse=self.reuse):
            ...
            if cond is not None:
                b = self.dense(cond, num_filters=out_shape[-1], activation="linear", batch_norm=False,
                               name=name + "w_cond_desne")
            else:
    ...
    def linear_conv(self, x, shape, stride=1, padding="SAME", name=None, cond=None):
        ...
        with tf.variable_scope(name, reuse=self.reuse):
            ...
            out_shape = out.get_shape().as_list()
            if cond is not None:
                b = self.dense(cond, num_filters=out_shape[3], activation="linear", batch_norm=False,
                               name=name + "w_cond_conv")
    ...
    def conv(self, x, filter_width, num_filters, stride=1, padding="SAME", name=None, activation=None, batch_norm=False,
             batch_norm_old=False, cond=None):
        if name is None:
    ...
    def dense(self, x, activation, num_filters,
              gaussian_noise=False, keep_prob=False, name=None, batch_norm=False, batch_norm_old=False, init="normal",
              cond=None):
        if name is None:
    ...
    def conv_right(self, x, num_filters, payload=None, filter_size=[4, 4], stride=1, name=None, activation=None,
                   mask="a", cond=None):
...
class CouplingLayer(object):
    ...
    def forward(self, x):
        ...
        for i in xrange(self.sub_layers - 1):
            f_x2 = self.parent.conv(f_x2, filter_width=self.filter_width, num_filters=self.num_filters, padding="SAME",
                                    activation=self.activation, name=self.name + "fx2_%d" % i)
        f_x2 = self.parent.conv(f_x2, filter_width=self.filter_width, num_filters=n_filter // 2, padding="SAME",
                                activation=self.activation, name=self.name + "fx2_" + str(self.sub_layers - 1))

        y1 = f_x2 + x1

        f_y1 = y1
        for i in xrange(self.sub_layers - 1):
            f_y1 = self.parent.conv(f_y1, filter_width=self.filter_width, num_filters=self.num_filters, padding="SAME",
                                    activation=self.activation, name=self.name + "fy1_%d" % i)
        f_y1 = self.parent.conv(f_y1, filter_width=self.filter_width, num_filters=n_filter // 2, padding="SAME",
                                activation=self.activation, name=self.name + "fy1_" + str(self.sub_layers - 1))

        y2 = f_y1 + x2
        out = self.combine(y1, y2)
        print ("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%", self.name, " Shape=",
               self.int_shape(out))
        return out
    ...
    def backward(self, y):
        ...
        f_y1 = y1
        for i in xrange(self.sub_layers - 1):
            f_y1 = self.parent.conv(f_y1, filter_width=5, num_filters=self.num_filters, padding="SAME",
                                    activation=self.activation, name=self.name + "fy1_%d" % i)
        f_y1 = self.parent.conv(f_y1, filter_width=5, num_filters=2, padding="SAME", activation=self.activation,
                                name=self.name + "fy1_" + str(self.sub_layers - 1))

        x2 = y2 - f_y1

        f_x2 = x2
        for i in xrange(self.sub_layers - 1):
            f_x2 = self.parent.conv(f_x2, filter_width=5, num_filters=self.num_filters, padding="SAME",
                                    activation=self.activation, name=self.name + "fx2_%d" % i)
        f_x2 = self.parent.conv(f_x2, filter_width=5, num_filters=2, padding="SAME", activation=self.activation,
                                name=self.name + "fx2_" + str(self.sub_layers - 1))
...
class RevDenseMask(object):
    ...
    def forward(self, x):
        ...
        f_x2 = x2
        for i in xrange(self.sub_layers - 1):
            f_x2 = self.parent.dense(f_x2, num_filters=self.num_filters, activation=self.activation,
                                     name=self.name + "fx2_%d" % i)
        f_x2 = self.parent.dense(f_x2, num_filters=n_filter // 2, activation=self.activation,
                                 name=self.name + "fx2_" + str(self.sub_layers - 1))
        ...
        f_y1 = y1
        for i in xrange(self.sub_layers - 1):
            f_y1 = self.parent.dense(f_y1, num_filters=self.num_filters, activation=self.activation,
                                     name=self.name + "fy1_%d" % i)
        f_y1 = self.parent.dense(f_y1, num_filters=n_filter // 2, activation=self.activation,
                                 name=self.name + "fy1_" + str(self.sub_layers - 1))
...
class CouplingDenseOld(object):
    ...
    def forward(self, x):
        ...
        return (
            self.mask * x + (1 - self.mask) * (x * tf.check_numerics(tf.exp(scale), "exp has NaN") + translation),
            scale)
    ...
    def f_scale(self, x):
        ...
        out = self.parent.dense(out, num_filters=n_filter, activation="tanh",
                                name=self.name + "_sub_s_" + str(self.sub_layers - 1))
    ...
    def f_translation(self, x):
        ...
        out = self.parent.dense(out, num_filters=n_filter, activation="linear",
                                name=self.name + "_sub_t_" + str(self.sub_layers - 1))
        return out
...
class CouplingRevNet(object):
    ...
    def forward(self, x):
        ...
        for i in xrange(self.sub_layers - 1):
            f_x2 = self.parent.dense(f_x2, num_filters=self.num_filters, activation=self.activation,
                                     name=self.name + "fx2_%d" % i, batch_norm=True, init=self.init)
        f_x2 = self.parent.dense(f_x2, num_filters=n_filter // 2, activation="linear",
                                 name=self.name + "fx2_" + str(self.sub_layers - 1), batch_norm=True, init=self.init)
        ...
        for i in xrange(self.sub_layers - 1):
            f_y1 = self.parent.dense(f_y1, num_filters=self.num_filters, activation=self.activation,
                                     name=self.name + "fy1_%d" % i, batch_norm=True, init=self.init)
        f_y1 = self.parent.dense(f_y1, num_filters=n_filter // 2, activation="linear",
                                 name=self.name + "fy1_" + str(self.sub_layers - 1), batch_norm=True, init=self.init)
...
class CouplingRevNet2(object):
    ...
    def backward(self, x):
        ...
        for i in xrange(self.sub_layers - 1):
            f_x2 = self.parent.dense(f_x2, num_filters=self.num_filters, activation=self.activation,
                                     name=self.name + "fx2_%d" % i, init=self.init)
        f_x2 = self.parent.dense(f_x2, num_filters=n_filter // 2, activation="linear",
                                 name=self.name + "fx2_" + str(self.sub_layers - 1), init=self.init)
        ...
        for i in xrange(self.sub_layers - 1):
            f_y1 = self.parent.dense(f_y1, num_filters=self.num_filters, activation=self.activation,
                                     name=self.name + "fy1_%d" % i, init=self.init)
        f_y1 = self.parent.dense(f_y1, num_filters=n_filter // 2, activation="linear",
                                 name=self.name + "fy1_" + str(self.sub_layers - 1), init=self.init)
...
class CouplingRevNetScale(object):
    def __init__(self, name, sub_layers, mask, num_filters=16, activation="relu", activation_last="linear", parent=None,
                 init="normal"):
        self.name = name + "_"
        self.parent = parent
        self.sub_layers = sub_layers
    ...
    def f_trans_x2(self, x2):
        out = x2
        for i in xrange(self.sub_layers - 1):
            out = self.parent.dense(out, num_filters=self.num_filters, activation=self.activation,
                                    name=self.name + "f_trans_x2%d" % i, init=self.init)
        out = self.parent.dense(out, num_filters=self.n_filter // 2, activation=self.activation_last,
                                name=self.name + "f_trans_x2" + str(self.sub_layers - 1), init=self.init)
        return out
    ...
    def f_scale_x2(self, x2):
        ...
        for i in xrange(self.sub_layers - 1):
            out = self.parent.dense(out, num_filters=self.num_filters, activation=self.activation,
                                    name=self.name + "f_scale_x2%d" % i)
        out = self.parent.dense(out, num_filters=self.n_filter // 2, activation=self.activation_last,
                                name=self.name + "f_scale_x2" + str(self.sub_layers - 1))
        return out
    ...
    def f_trans_y1(self, y1):
        ...
        for i in xrange(self.sub_layers - 1):
            out = self.parent.dense(out, num_filters=self.num_filters, activation=self.activation,
                                    name=self.name + "f_trans_y1%d" % i, init=self.init)
        out = self.parent.dense(out, num_filters=self.n_filter // 2, activation=self.activation_last,
                                name=self.name + "f_trans_y1" + str(self.sub_layers - 1), init=self.init)
        return out
    ...
    def f_scale_y1(self, y1):
        ...
        for i in xrange(self.sub_layers - 1):
            out = self.parent.dense(out, num_filters=self.num_filters, activation=self.activation,
                                    name=self.name + "f_scale_y1%d" % i)
        out = self.parent.dense(out, num_filters=self.n_filter // 2, activation=self.activation_last,
                                name=self.name + "f_scale_y1" + str(self.sub_layers - 1))
        return out
...
class CouplingConvOld(object):
    ...
    def f_trans_x2(self, x2):
        ...
        out = self.parent.conv(out, filter_width=self.filter_width, num_filters=self.n_filter // 2,
                               ...
                               name=self.name + "f_trans_x2" + str(self.sub_layers - 1))
        return out
    ...
    def f_trans_y1(self, y1):
        ...
        out = self.parent.conv(out, filter_width=self.filter_width, num_filters=self.n_filter // 2,
                               activation=self.activation_last,
                               name=self.name + "f_trans_y1" + str(self.sub_layers - 1))
        return out
...
class CouplingDense(object):
    ...
    def f_trans_x2(self, x2):
        ...
        for i in xrange(self.sub_layers - 1):
            out = self.parent.dense(out, num_filters=self.num_filters, activation=self.activation,
                                    name=self.name + "f_trans_x2%d" % i, batch_norm=self.batch_norm, init=self.init)
        out = self.parent.dense(out, num_filters=self.n_last_filter, activation=self.activation_last,
                                name=self.name + "f_trans_x2" + str(self.sub_layers - 1),
                                batch_norm=self.batch_norm_last, init=self.init)
        return out
    ...
    def f_trans_y1(self, y1):
        ...
        for i in xrange(self.sub_layers - 1):
            out = self.parent.dense(out, num_filters=self.num_filters, activation=self.activation,
                                    name=self.name + "f_trans_y1%d" % i, batch_norm=self.batch_norm, init=self.init)
        out = self.parent.dense(out, num_filters=self.n_last_filter, activation=self.activation_last,
                                name=self.name + "f_trans_y1" + str(self.sub_layers - 1),
                                batch_norm=self.batch_norm_last, init=self.init)
        return out
...
class CouplingConv(object):
    ...
    def f_trans_x2(self, x2):
        out = x2
        for i in xrange(self.sub_layers - 1):
            out = self.parent.conv(out, num_filters=self.num_filters, filter_width=self.filter_width,
                                   activation=self.activation, name=self.name + "f_trans_x2%d" % i,
                                   batch_norm=self.batch_norm)
        out = self.parent.conv(out, num_filters=self.n_last_filter, filter_width=self.filter_width_last,
                               activation=self.activation_last,
                               name=self.name + "f_trans_x2" + str(self.sub_layers - 1),
                               batch_norm=self.batch_norm_last)
        return out
    ...
    def f_trans_y1(self, y1):
        ...
        for i in xrange(self.sub_layers - 1):
            out = self.parent.conv(out, num_filters=self.num_filters, filter_width=self.filter_width,
                                   activation=self.activation, name=self.name + "f_trans_y1%d" % i,
                                   batch_norm=self.batch_norm)
        out = self.parent.conv(out, num_filters=self.n_last_filter, filter_width=self.filter_width_last,
                               activation=self.activation_last,
                               name=self.name + "f_trans_y1" + str(self.sub_layers - 1),
...
class CouplingDeConv(object):
    ...
    def f_trans_x2(self, x2):
        out = x2
        for i in xrange(self.sub_layers - 1):
            out = self.parent.deconv(out, output_shape=self.output_shape, num_filters=self.num_filters,
                                     filter_width=self.filter_width, activation=self.activation,
                                     name=self.name + "f_trans_x2%d" % i, batch_norm=self.batch_norm)
        out = self.parent.deconv(out, output_shape=self.output_shape, num_filters=self.n_last_filter,
                                 filter_width=self.filter_width_last, activation=self.activation_last,
                                 name=self.name + "f_trans_x2" + str(self.sub_layers - 1),
                                 batch_norm=self.batch_norm_last)
        return out
    ...
    def f_trans_y1(self, y1):
        out = y1
        for i in xrange(self.sub_layers - 1):
            out = self.parent.deconv(out, num_filters=self.num_filters, filter_width=self.filter_width,
                                     activation=self.activation, name=self.name + "f_trans_y1%d" % i,
                                     batch_norm=self.batch_norm)
        out = self.parent.deconv(out, num_filters=self.n_last_filter, filter_width=self.filter_width_last,
                                 activation=self.activation_last,
                                 name=self.name + "f_trans_y1" + str(self.sub_layers - 1),
                                 batch_norm=self.batch_norm_last)
        return out
